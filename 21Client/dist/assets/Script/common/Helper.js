...
Helper.JsBridge = function (isPortrait, callback) {
    ...
    if (cc.sys.isBrowser) {
        ...
        if (isPortrait) cc.view.setOrientation(cc.macro.ORIENTATION_PORTRAIT);else cc.view.setOrientation(cc.macro.ORIENTATION_LANDSCAPE);

        callback && callback();
        return;
    }

    var num = isPortrait === true ? 2 : 1;
...
Helper.loadGameScene = function () {
    Helper.JsBridge(false, function () {
...
Helper.wxShareWebUrl = function (url, title, content, isTimeLine) {
    ...
    if (cc.sys.os === cc.sys.OS_ANDROID) {
        jsb.reflection.callStaticMethod("cn/dapenggame/lxsss/SDKManager", "shareWebUrl", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Z)V", url, title, content, isTimeLine);
    } else if (cc.sys.os === cc.sys.OS_IOS) {
