...
cc.Class({
    ...
    onLoad: function onLoad() {
        // 成员变量
        this.POINT_IN = cc.p(950, 650), this.POINT_OUT = cc.p(350, 650), this._rank = 0; // 卡牌点数
    ...
    * */
    // 加载卡牌
    loadCards: function loadCards(cardData, isDeal) {
    ...
    // 加载点数标签
    loadRankLabel: function loadRankLabel() {
    ...
    // 加载箭头
    loadArrowSpr: function loadArrowSpr() {
        ...
        positionB.y += 15;
        this._arrowSpr.runAction(cc.repeatForever(cc.sequence(cc.moveTo(0.5, positionB), cc.moveTo(0.5, positionA))));
    },

    // 加载骨骼动画
    loadArmature: function loadArmature(result) {
        var scale = 1.0;
        var p = cc.p(0, -40);
        ...
        // 播放牌型提示动画
        if (armatureName) {
            this._armature = Util.playAnimation(armatureName, animationName, null, loop, 0).to(this).p(p).qscale(scale);
        }
    },

    // 卡牌构造器
    cardMaker: function cardMaker(cardData, delayMove) {
    ...
    //增加一个牌的节点
    addCardNode: function addCardNode(img) {
    ...
    //增加一个节点
    addDoubleTip: function addDoubleTip(img) {
        var node = new cc.Node(img);
    ...
    * */
    // 更新卡牌信息
    updateCards: function updateCards(data) {
    ...
    // 更新点数等内容
    updateContents: function updateContents(result) {
        var visible = true;
    ...
    //设置点数内容
    setRankStr: function setRankStr(rankStr) {
        this._rankLab.string = rankStr;
        this._rankBgSpr.active = true;
    },
    //设置可见性
    setSprVisible: function setSprVisible(spr, visible) {
        spr.active = visible;
    },
    // 设置箭头的可见性
    setArrowVisible: function setArrowVisible(visible) {
        this._arrowSpr.active = visible;
    },

    //更新点数的背景图
    setRankSprSprite: function setRankSprSprite(img) {
    ...
    // 显示唯一的点数
    showOnlyRank: function showOnlyRank(curCards) {
    ...
    showCard: function showCard(data) {
        ...
        cardSpr.runAction(cc.sequence(cc.scaleTo(0.25, 0, 1), cc.callFunc(function () {
            var p = cardSpr.getPosition();
            var newCardSpr = new CardSprite(card).to(this).p(p).qscale(0, 1);

            newCardSpr.runAction(cc.sequence(cc.scaleTo(0.25, 1), cc.callFunc(function () {
                // 更新点数等内容
                this.updateContents(data.curCards.result);
                // 排列卡牌
                this.orderCards();
    ...
    blackJackJudge: function blackJackJudge(cardSpr) {
        cardSpr.runAction(cc.sequence(cc.delayTime(1.5), cc.moveBy(0.1, cc.p(0, 20)), cc.delayTime(1.0), cc.moveBy(0.1, cc.p(0, -20))));
    },
    ...
    // 卡牌移动.
    cardMove: function cardMove(cardSpr, targetP, delayMove, scale, rotate) {
        cardSpr.runAction(cc.sequence(cc.delayTime(delayMove),
        // cc.callFunc(function(){
        //     SoundEngine.playEffect("res/21dian/sound/DealPoker.mp3");
        // }.bind(this)),
        cc.spawn(cc.scaleTo(0.5, scale), cc.moveTo(0.5, targetP), cc.rotateBy(0.5, rotate))));
    },
...
//# sourceMappingURL=CardBoxSprite.js.map