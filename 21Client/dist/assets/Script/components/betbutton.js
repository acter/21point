...
cc.Class({
    ...
    loadInfo: function loadInfo(mainScene) {
        ...
        this._betButtons = [];
        this._otherButtons = [];
        this._operateButtons = [], this.TYPE_BTN_MAX = 0;
    ...
    // 显示下注层
    showBetLayer: function showBetLayer(data, myScore) {
    ...
    // 显示操作层
    showOperateLayer: function showOperateLayer() {
        this.operateContainer.active = true;
        this.betbtn_list.active = false;
        this.setOperateLayerVisible(true);
    },
    // 设置下注层的按钮可点击
    setBetLayerVisible: function setBetLayerVisible(visible) {
        // 如果可见,则恢复正常且可点击
        visible && this.setBetButtonsOpacity(255);
    },
    //设置操作按钮的可点击
    setOperateLayerVisible: function setOperateLayerVisible(visiable) {
        visiable && this.setOperateButtonsOpacity(255);
    },
    // 设置按钮的透明度
    setButtonsOpacity: function setButtonsOpacity(buttons, opacity) {
    ...
    //设置按钮的可用性
    setButtonEnable: function setButtonEnable(button, flag) {
        button.interactable = flag;
    },
    // 设置下注按钮的透明度
    setBetButtonsOpacity: function setBetButtonsOpacity(opacity) {
        this.setButtonsOpacity(this._betButtons.concat(this._otherButtons), opacity);
    },
    //设置操作按钮的透明度
    setOperateButtonsOpacity: function setOperateButtonsOpacity(opacity) {
    ...
    /*
       ...
       * */
    // 加载下注层按钮
    loadBetButtons: function loadBetButtons() {
        var bets = this.ALL_BETS[this.mainScene.roomIndex];
        var types = [this.TYPE_BTN_FIRST, this.TYPE_BTN_SECOND, this.TYPE_BTN_THIRD, this.TYPE_BTN_FOURTH, this.TYPE_BTN_FIFTH];
    ...
    // 加载其他按钮
    loadOtherButtons: function loadOtherButtons() {
    ...
    // 加载操作层按钮
    loadOperateButtons: function loadOperateButtons() {
        var names = ["split", "double", "stand", "hit"];
        var types = [this.TYPE_BTN_SPLIT, this.TYPE_BTN_DOUBLE, this.TYPE_BTN_STAND, this.TYPE_BTN_HIT];
    ...
    // 点击最大下注数按钮的梳理函数
    onMaxButtonHandler: function onMaxButtonHandler(btn) {
    ...
    // 点击上局下注按钮的处理函数
    onLastButtonHandler: function onLastButtonHandler(btn) {
    ...
    onBetButtonHandler: function onBetButtonHandler(btn) {
    ...
    // 发送下注信息
    sendBetMessage: function sendBetMessage(betNum) {
    ...
    onOperateButtonHandler: function onOperateButtonHandler(btn) {
        var msg = "";
        switch (btn.target.type) {
            case this.TYPE_BTN_SPLIT:
    ...
    //发送操作消息
    sendOperateMessage: function sendOperateMessage(msg, data) {

        GD.clientKernel.sendSocketData(gameCMD.MDM_GF_GAME, msg, data);
    },
    // 发送下注信息
    sendWagerMessage: function sendWagerMessage(betNum) {
        var curBets = this.ALL_BETS[this.mainScene.roomIndex];
        var maxBet = curBets[curBets.length - 1];
        betNum >= maxBet && this.hideBetButton();
        this._lastBetNum = betNum;
    ...
    //隐藏下注层
    hideBetButton: function hideBetButton() {
        this.betbtn_list.active = false;
    },

    // 当前下注与按钮的值相加超过最大值的不能点
    setBetButtonsDisabled: function setBetButtonsDisabled() {
        var curBets = this.ALL_BETS[this.mainScene.roomIndex];
        var maxBet = curBets[curBets.length - 1];
    ...
    setSplitAndDouble: function setSplitAndDouble(canSplit, canDouble) {
    ...
    findButtonByType: function findButtonByType(type) {
