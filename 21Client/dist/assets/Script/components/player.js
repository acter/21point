...
cc.Class({
    ...
    // 设置底注
    setBaseBet: function setBaseBet(betNum) {
    ...
    // 下注的处理函数
    onWagerHandler: function onWagerHandler(data, arr) {
        ...
        var iteration = function (betNum, arr) {
            ...
            for (var j = 0; j < intNum; j++) {
                ...
                (function (node, j) {
                    ...
                    node.runAction(cc.sequence(cc.delayTime(0.2 * j), cc.moveTo(0.2, cc.p(x, y))));
                })(node, j);
    ...
    // 要牌的处理函数
    onHitHandler: function onHitHandler(data, isDealer) {
    ...
    // 金币移动
    coinMove: function coinMove(num) {
        ...
        coinNode.runAction(cc.spawn(cc.moveBy(1.0, cc.p(0, 100)), cc.sequence(cc.delayTime(0.5), cc.fadeOut(0.5), cc.callFunc(function () {
            coinNode.active = 0;
        }.bind(this)))));
    },
    // 加载筹码
    loadBet: function loadBet(betNum) {
        if (betNum != 0) {
    ...
    // 加载套牌
    loadCardBoxes: function loadCardBoxes(cardData, dealID) {
    ...
    // 显示要牌的箭头提示
    setHitTipVisible: function setHitTipVisible(targetKey, visible) {
    ...
    //更改玩家分数
    setUserScore: function setUserScore(score) {
    ...
    //设置玩家是否在玩状态
    setIsPlaying: function setIsPlaying(flag) {
