...
cc.Class({
    ...
    startSprIn: function startSprIn() {
        ...
        this.startSpr.runAction(cc.sequence(cc.moveTo(0.5, cc.p(0, 0)).easing(cc.easeBackOut(0.5)), cc.delayTime(0.5), cc.moveTo(0.5, cc.p(x, 0)).easing(cc.easeBackIn(0.5)), cc.callFunc(function () {
    ...
    * */
    ...
    //加载玩家座位
    playerSit: function playerSit(chairID, data) {
    ...
    // 加载房间信息层
    loadRoomInfoLayer: function loadRoomInfoLayer(data) {
    ...
    // 发牌的处理函数
    onDealHandler: function onDealHandler(data) {
        var count = 0;
    ...
    // 下一个玩家行动
    onNextActionHandler: function onNextActionHandler(data) {
    ...
    // 提示保险的处理函数
    onInsuranceHandler: function onInsuranceHandler(data) {
        var myUserItem = GD.clientKernel.myUserItem;
        ...
        for (var i = 0, lenI = this.players.length; i < lenI; i++) {
            ...
            if (isPlaying && !player.isDealer()) {
                if (myUserItem.userID == player.getUserID()) {
                    this.insuranceLayer = new InsuranceLayer(this.onCloseInsuranceLayer.bind(this), betNum).to(this, 2);
                }
    ...
    // 买保险的处理函数
    onBuyInsuranceHandler: function onBuyInsuranceHandler(data) {
    ...
    // 关闭强制退出提示
    onCloseInsuranceLayer: function onCloseInsuranceLayer(type) {
    ...
    // 要牌的处理函数
    onHitHandler: function onHitHandler(data) {
