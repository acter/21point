...
U.numberMakerWithConfig = function (img, num, config, isEnglish, isInt, isShort) {
    ...
    var numberTextMaker = function numberTextMaker(num, isEnglish, isInt) {
        var s = "";
        var base = 1;
...
U.copyArray = function (sourceArr, targetArr) {
    targetArr = targetArr || [];
    var iteration = function iteration(sourceArr, copyArr) {
        copyArr = copyArr || [];
...
U.countCard = function (cards, hard, canBlackJack) {
    ...
    var isBlackJack = canBlackJack && cardNum == 2 && (cards[0].num == 1 && cards[1].num >= 10 || cards[0].num >= 10 && cards[1].num == 1);
