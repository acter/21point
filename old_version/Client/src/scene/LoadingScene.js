...
var LoadingScene = FocusLoadingScene.extend({
        ...
        cc.loader.load(self.loadArray
            ...
            , function () {
                ...
                if (self.finishCallback) {
                    ...
                    //释放资源
                    //cc.spriteFrameCache.removeSpriteFramesFromFile(loadingRes.LOADING_PLIST);
                }
            });
    },
    ...
    initEx: function () {
        ...
        this.progress = new ComLoadingLayer().to(this).pp();

        this.setContentSize(cc.winSize);

        this.logoBG = new cc.Sprite("#res/21dian/loading/logoBg.png").to(this).pp(0.5, 0.6);

        var size = this.logoBG.getContentSize();

        this.cardColors = new cc.Sprite("#res/21dian/loading/" + this.colorName[this.index]).to(this.logoBG).p(size.width / 2 + 26, size.height / 2 + 126);
        this.cardColors.setScale(0.8);

        this.logo = new cc.Sprite("#res/21dian/loading/logo.png").to(this.logoBG).pp(0.508, 0.572);
        this.logo.setScale(0.9);

        this.loadTS = new cc.Sprite("#res/21dian/loading/loading.png").to(this.logoBG).pp(0.46, -0.1);
        this.loadTS.setScale(0.8);

        for (var i = 0; i < 3; i++) {
            this.loadPointArray[i] = new cc.Sprite("#res/21dian/loading/point.png").to(this.loadTS).pp(1.1 + i * 0.1, 0.2);
        }
    ...
    updateBling: function () {
        ...
        this.cardColors.display("#res/21dian/loading/" + this.colorName[index]);
